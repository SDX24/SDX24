...
- Optionally render a top-right triangular `Rationale` badge for inline rationale toggles
- Optionally render a compact `Case Study` badge near the title block
...
- `showLogo` (optional logo visibility override)
- `caseStudyBadge` (optional compact badge copy)
- `onRationaleToggle`, `isRationaleOpen`, `rationaleBadgeClassName` (optional rationale badge controls + styling)
...
- Optionally render a large right-side case-study badge attached to the card container so it follows the same scroll/landing transforms
- Support optional back-face interaction modes:
  - legacy hover expansion via `HeroBackHoverCard`
  - landed focus behavior via `ProjectFocusCard`
- Support optional landed focus interaction guardrails (hover arm delay and click-to-open)
...
- Use `enableLandedFocusEffect` to switch landed interaction to `ProjectFocusCard`
- Use `enableBackHoverExpand={false}` when landed focus is active to avoid dual interaction ownership
...
- Render a responsive projects grid with uniform compact-card footprints
- Populate cards with Rudi, InsurFlow, Adult, Bandit Breakout, and Express Documentation via `ProjectFocusCard`
- Support inline rationale behavior: clicking a card’s `Rationale` badge inserts a matching description panel immediately after that card in the grid
- Style inline rationale panels with per-project color treatments derived from project/logo visual language

**Usage rules**:

- Keep heading natural-scrolling (no heading pinning)
- Pass `showCompactLinks` to surface compact links
- Keep rationale behavior inline in the grid (no dedicated Rationales section below Projects)
...
- Pass optional compact-card rationale props through to `ProjectCardCompact`
...
- `showLogoOnCompact`, `caseStudyBadge`, `onRationaleToggle`, `isRationaleOpen`, `rationaleBadgeClassName` (optional compact-card passthrough controls)
...
- Render fullscreen content with a sticky top header (back icon + centered brand marks), project logo, and expanded detail blocks
- Render optional long-form case-study content in fullscreen when project data includes structured case-study sections
- Support case-study presentation enhancements (narrative map rail, per-section gradient variants, section visuals, and embedded live preview)
- Support optional rationale-toggle passthrough for compact grid cards (`showRationaleToggle`, `onRationaleToggle`, `isRationaleOpen`)
...
- Support case-study labeling hooks for hero usage via optional selectors (`data-case-study-fullscreen`, `data-case-study-back`)
- Support portal fullscreen rendering (`renderFullscreenInPortal`) for transformed/stacked parent contexts (hero landed state)
- Support direct click-to-open fullscreen behavior alongside hover/tap priming
- Support interaction arm delay behavior for hover activation edge cases (`interactionArmDelayMs`)
- Allow semantic naming of the opened view via `openViewLabel` (`"fullscreen" | "case-study"`) for accessibility labels and selectors
- Support case-study metadata blocks (role, timeline, team, stack, links), sectioned narrative, outcomes, team credits, and references when provided in `project` data
- Render timeline as a minimal static progress rail with week markers (`4`, `8`, `12`, `16`) and a clickable team summary block that scrolls to Team Credits
- Render project links in centered action cards and support a live-status tag on the primary live-link card
- Support collapsible Tech Stack behavior: collapsed chip row preview (~6 chips) and expanded full-chip grid
- Render live preview section as a centered embedded frame with a lightweight browser-style header
- Use click-only preview interactions for case-study assets with section-scoped expand/collapse behavior and smooth low-bounce transitions
- Render section visual assets from either `figmaEmbedUrl` (iframe) or `src` (image) using the same expand/collapse container
- For Figma assets, crop the embedded frame so left-side Figma UI selector panels are not visible in expanded previews
- Keep team-credit card labels focused on team tags (`Development`/`Design`) with optional LinkedIn/Portfolio/GitHub links and optional avatar image
...
- Use `renderFullscreenInPortal` when card exists inside transformed containers (e.g., hero back face) to guarantee viewport fullscreen
- For hero landed case-study cards, click should open fullscreen immediately while hover remains available
- For hero landed case-study cards, set `openViewLabel="case-study"` so UI labels and selectors use case-study semantics
- For long-form case-study cards, provide section assets/citations as data and avoid section-level `resourceHref`/`resourceLabel` wrappers
- For section-specific visual identity, optionally provide `gradientVariant` (`sky|mint|teal|slate|amber`) in each case-study section
- For team-credit segmentation, optionally provide `team` (`Design|Development`) in case-study credits
